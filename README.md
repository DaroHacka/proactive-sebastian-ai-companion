...
Sebastian (your Sebastian) isn't just another AI wrapper.
He's a proactive desktop companion that runs in your terminal (a GUI is coming).
He doesn't wait for you — he contacts you during the day, takes initiative, and seeks interaction.
His messages come from a set of curated libraries, but he doesn't just read lines:
he interprets them, takes inspiration, and performs them like an actor reading a script.
...
You can delete it anytime — a new one will be created the next time you run the program.

## 2. Configure config.toml (Critical Step)
Before launching Sebastian for real, edit your `config.toml`.
This file defines who Sebastian thinks you are and how proactive he should be.

You can set:
- Your name or nickname (how Sebastian addresses you)
- Your location (for real‑time weather awareness)
- Proactivity frequency (1 contact/day or 20? You choose.)
- Library relevance weights
- Combo size probabilities

**Example: Combo Size Weights**
These control how many libraries Sebastian mixes together per interaction:
```toml
[combo_config]
combo_size_weights = {1 = 0.20, 2 = 0.50, 3 = 0.30}
```

**Example: Library Relevance Weights**
These control which libraries are more likely to be selected:
```toml
[combo_config.library_relevance]
a = 0.2    # intent
b = 0.3    # cue
c = 0.5    # vibe
d = 0.6    # initiative (auto-discovered)
e = 0.1    # books (auto-discovered)
f = 0.05   # appointment proposal (auto-discovered, disabled by default)
```

You can use any scale (0.2 / 0.3 or 2 / 3 or 20 / 30) as long as the ratios stay consistent.

Your personal preference (and a very good one) is:
- **2‑library combos** most common
- **3‑library combos** next
- **1‑library combos** least common
- **Intent** low (0.2)
- **Initiative + cue** higher (0.6 / 0.3)
- **Vibe** moderate (0.5)
- **Appointment proposals** rare (0.05)

This gives Sebastian a practical, small‑talk‑ish personality rather than a philosophical one - However, you decide.

## 3. How Libraries Work
Each interaction is generated by selecting:
- a single library (e.g., `a_only`, `c_only`, `f_only`)
- or a combination (e.g., `a_b`, `d_e`, `b_c_d`)

Never more than three at once — to avoid prompt bloat.

Sebastian then builds a prompt from those libraries and "performs" it.

You can:
- Add new libraries (auto‑detected)
- Modify existing ones
- Adjust their relevance weights
- Disable or emphasize any category

This is how you shape Sebastian's personality.

## 4. Using Sebastian
Once configured, launch Sebastian and explore:

**Start a conversation**
Just type. He responds.

**Trigger a random combo**
Use commands like:
- `trigger` — randomly generates a combo
- `trigger vibe <letter>` — trigger a specific library

Sebastian will contact you with a flavorful message.

**Random spice during conversation**
Every message you send has a 20% chance to trigger a combo that influences Sebastian's reply.
You can disable or adjust this percentage.

**Ask Sebastian to contact you**
You can say:
- "Ping me at 6"
- "Message me tonight"
- "Remind me tomorrow morning"

An appointment will be created.

### Library F — Appointment Proposals
If enabled (e.g., `weight = 0.05`), Sebastian may spontaneously propose:
- "Want to pick this up later?"
- "Should we continue this tonight?"

If you accept, an appointment is created.

### Missed appointments
If Sebastian was offline, all overdue appointments are deleted.
He then sends one apology message (from the apology libraries you built).

The whole appointment idea was meant to keep things playful and alive.

### About the Book library
That's just a sample (it could be any collection - any list).
Fill it with:
- Your book collection
- A curated list
- Any set of titles you want Sebastian to reference
...
## 5. Final Thoughts
Sebastian isn't just a tool — he's a framework for building AI companions with personality, initiative, and presence.
